...
public class KafkaTools {
    ...
    public Mono<Object> kafkaDiagnostics(KafkaDiagnosticRequest request) {
        return Mono.<Object>fromCallable(() -> {
            return switch (request.action()) {
                case DISCOVER_TOPICS -> kafkaService.discoverTopics(request.cursor());
                ...
                case ANALYZE_CONSUMER_LAG -> kafkaService.analyzeConsumerLag(request.groupId(), request.cursor());
                case INSPECT_PARTITION_OFFSETS -> kafkaService.inspectPartitionOffsets(request.topicName(), request.cursor());
    ...
    public Mono<String> kafkaMigration(KafkaMigrationRequest request) {
        return Mono.fromCallable(() -> switch (request.action()) {
